...
def calculate_visit_indices(conn, visit_id, clear_if_empty=False):
    """
    Recalculate macro_analysis for a single visit using the same formulas as the
    global BAT index job. Prefer rbp100_bug when sum(amount) >= 50; otherwise
    use non-excluded bug_count.

    Returns True if a macro_analysis row was upserted, False if insufficient data.
    When clear_if_empty is True and data is insufficient, deletes any existing
    macro_analysis row for this visit only (used by staff data-entry after edits).
    Does not commit; caller owns the transaction.
    """
    cur = conn.cursor()
    cur.execute(
        """
        SELECT v.visit_id, v.site_id, s.drainage_area_sq_km
        FROM visit v
    ...
    _visit_id, _site_id, drainage_area_sq_km = visit_row
    ...
    if not rows:
        if clear_if_empty:
            cur.execute("DELETE FROM macro_analysis WHERE visit_id = %s", (visit_id,))
        return False
    ...
    if not dom and not use_rbp:
        cur.execute(
            ...
            FROM bug_count bc JOIN bug_list b ON b.bug_id = bc.bug_id
            WHERE bc.visit_id = %s AND bc.exclude = false
...
def run():
    with get_conn() as conn:
        ...
        for visit_id in visits:
            # ETL path: skip empty visits without clearing (preserve historical rows)
            calculate_visit_indices(conn, visit_id, clear_if_empty=False)
