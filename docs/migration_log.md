...
**IDEXX bacteria idempotency:** `etl/migrate_bact_2025.py` attaches IDEXX rows by `visit.sample_code` and skips when `(visit_id, e_coli_mpn_100ml)` already exists. Re-runs must not multiply bacteria rows. Distinct MPN values on the same sample code remain representable (no broad UNIQUE). Historical ALL DATA `E. coli Result` is loaded by `migrate_streamwatch_data` (modifier text → `detection_limit_note`); it is **not** loaded by the IDEXX path. Censored IDEXX values (`> 2419.6`, `< 1.0`) remain unparsed/skipped. Gallery lab sheets remain unloaded.
